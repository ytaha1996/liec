...
const SupplyOrdersPage = () => {
    ...
    const lifecycleActionDefs = LIFECYCLE_ACTIONS.map(({ label, action }) => ({
        icon: _jsx(PlayArrowIcon, { fontSize: "small" }),
        label,
        onClick: (id) => lifecycleMut.mutate({ id, action }),
        hidden: () => false,
    }));
    const tableHeaders = [
    ...
    return (_jsxs(Box, { children: [_jsx(MainPageTitle, { title: "Supply Orders", action: {
                    title: 'Create Supply Order',
                    onClick: () => { setEditing(null); setDialogOpen(true); },
                } }), _jsx(Box, { sx: { px: 3, pb: 3 }, children: _jsx(EnhancedTable, { title: "Supply Orders", header: tableHeaders, data: tableData, defaultOrder: "name" }) }), _jsx(GenericDialog, { open: dialogOpen, title: editing ? 'Edit Supply Order' : 'Create Supply Order', onClose: () => { setDialogOpen(false); setEditing(null); }, children: _jsx(DynamicFormWidget, { title: "", drawerMode: true, fields: buildFields(editing ?? undefined), onSubmit: handleSubmit }) })] }));
};
