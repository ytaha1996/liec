...
import { CustomersPage, DashboardPage, LoginPage, MessagingLogsPage, PackageDetailPage, ShipmentsPage, SimpleListPage } from './pages/Pages';

const PkgWrap=()=>{const {id='0'}=useParams(); return <PackageDetailPage id={id}/>} 

export const App=()=> <Box><AppBar position='static'><Toolbar><Button color='inherit' component={Link} to='/'>Dashboard</Button><Button color='inherit' component={Link} to='/shipments'>Shipments</Button><Button color='inherit' component={Link} to='/customers'>Customers</Button><Button color='inherit' component={Link} to='/messaging-logs'>Messaging</Button></Toolbar></AppBar><Container sx={{py:2}}><Routes>
